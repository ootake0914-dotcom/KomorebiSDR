...
### 実験・採否 Verdict (測ってから決める原則の徹底)
1. **デジタル・ソフトサチュレーション (不採用・REJECT)**:
   - 目的: 8-bit ADCの矩形波クリッピングによる高調波・混変調歪みを tanh / 3次多項式で緩和する試み。
   - 実測結果: クリップ生波形 SINAD 10.77dB に対し、tanh 適用後 9.02dB (-1.76dB悪化)、3次多項式 9.04dB (-1.73dB悪化)。
   - 理由: すでにADCでハードクリップされて平坦になった波形に非線形関数を通しても平坦部は平坦なままであり、非飽和な線形領域まで歪んで全高調波歪みが増大するため。
   - 採否: **不採用（コード投入せず棄却）**。過入力対策はハードウェアゲイン自動引き下げ（Auto Gain Interlocking）が正解。
2. **AFC引き込み範囲の最適化 (採用・ACCEPTED)**:
   - 目的: 旧仕様で $\pm 20\text{kHz}$ にハードコードされていた制限により、$+25\text{kHz}$ 以上のオフセット（PPMドリフト・選局ズレ）で 0Hz 膠着（追従完全停止）していた問題を解消。
   - 対策: `self.afc_limit_hz = 42000.0`（$\pm 42\text{kHz}$）へ拡大。隣接局（100kHz間隔）への誤引き込みマージン（$>58\text{kHz}$）を安全に保ちつつ引き込み範囲を倍増。
   - 実測結果: Offset +25kHz において旧 `OUT_OF_RANGE (final_afc = +0Hz, 残差 25000Hz)` から `PULLING (final_afc = -13415Hz, 残差 11585Hz)` へ引き込み追従が正常作動。膠着を完全解消。
